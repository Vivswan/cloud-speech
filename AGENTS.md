...
- `apps/web`: Astro static site (setup guides at `setup/<provider>/`, pricing, troubleshooting, privacy policy) → GitHub Pages at vivswan.github.io/cloud-speech via the managed `pages.yml`: root = newest `vX.Y.Z` tag, `latest/` = main HEAD, one `vX.Y.Z/` per served tag (layout in repo-platform's docs/pages.md)
...
- Releases via release-please with `"versioning": "always-bump-patch"` (`release-please-config.json`): every release is a patch bump; `feat:`/`fix:` only sort the changelog, and a `Release-As: X.Y.Z` commit footer is the deliberate way to move minor or major. The store publish pipeline (2 CWS listings + AMO) lives in `.github/workflows/update-release.yml`.
