 ...
 *  A state, not a failure, so it keeps the note palette and never dismisses
...
export function NewerVersionNote({ storedVersion }: NewerVersionNoteProps) {
  return (
    <ErrorNotice
      error={describeNewerVersion(storedVersion)}
      className="border-note-edge bg-note text-note-text"
    />
  );
}
