...
| 9 | `09-settings-save-test-error.jpg`, `-2x` | The OpenAI card expanded after a failed `Save & test`: the key field, the failure (`Authentication failed. Re-copy the credentials and try again.`) with the `Details:` line under it carrying the HTTP 401 and OpenAI's own wording, the row's `Not connected` chip, and the Not connected rows around it (Google Cloud TTS above, OpenAI-compatible below), down to the card's bottom corners; the window starts in the gap above the Google Cloud TTS row | Runs first, before any provider is connected; a request carrying the scene's revoked key is answered with 401 and OpenAI's rejected-key error envelope instead of reaching the fake server |
